...
from poser.utils.urlutils import urljoin
...
def mail_page_user_change(user, created=False, password=""):
    ...
    """
    if created:
        subject = _('POSER - your user account was created.')
    else:
        subject = _('POSER - your user account was changed.')
    send_mail(subject, 'admin/poser/mail/page_user_change.txt', [user.email], {
        'user': user,
        'password': password or "*" * 8,
        'created': created,
    }, 'admin/poser/mail/page_user_change.html')
